...
### 결론  
- 기대한 것과 달리, 일별과 요일별로 상관관계를 분석했을 때 서울시의 교통량과 미세먼지 농도 간의 뚜렷한 선형관계는 파악되지 않음  
#### 즉, 서울시의 미세먼지 농도와 서울시의 교통량은 일별, 요일별로는 상관 관계가 없다는 결론을 내림 

- 그러나 연도 별로 분석했을 때, 2018년 2019년과 달리 2020년에는 다소 약하지만 유의미한 선형 관계가 파악됨  
> #### 2020년만 관계가 있게 나온 이유는?  
- 단순하게 데이터의 우연의 일치일 가능성  
- 2020년의 경우, 1~3월의 데이터만으로 분석을 진행하였는데, 분기별 혹은 월별에 따라 영향이 미치는 정도가 달라질 가능성    
> #### 일별, 요일별로는 교통량의 변화가 뚜렷하지 않아 상관관계가 없다고 판단되었을 가능성 있음 
#### :point_right: 주중 및 주말별로는 교통량의 변화가 뚜렷하므로 상관관계가 있지 않을까?   
#### 모든 가능성 및 요소들을 고려해 교통량-미세먼지 관련 자료 찾아본 후 추가적인 분석(주중 및 주말별) 진행 예정 
